...
function runRules(deck, setScope) {
   return DS.runRulesForDeck(deck, setScope, {});
}
...
describe('deck-suggest tagger fallback', () => {
   it('ranks cuts using tag overlap when tagger metadata present', () => {
      const deck = loadFixture('baird-snapshot.json');
      const setScope = loadFixture('set-msh-slice.json');
      const ctx = DS.Tagger.createContext(deck, setScope);
      const ranked = DS.RuleGuards.rankCutCandidates(
         DS.RuleGuards.cutCandidates(deck),
         deck.profile,
         ctx
      );
...
describe('deck-suggest commander eligibility', () => {
   it('skips cube decks by name', () => {
      ...
      const result = DS.Data.resolveDeckEligibility(deck);
