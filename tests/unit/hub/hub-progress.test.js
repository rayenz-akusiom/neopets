...
beforeEach(() => {
   ...
   HubProgress = window.HubProgress;
...
describe('HubProgress', () => {
   it('mount creates hidden bar in host', () => {
      ...
      const bar = host.querySelector('.hub-progress-bar');
   ...
   it('finish shows dismiss and success variant', () => {
      ...
      expect(document.querySelector('.hub-progress-dismiss').hidden).toBe(false);
      expect(controller.isActive()).toBe(false);
